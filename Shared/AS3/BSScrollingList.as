package Shared.AS3
{
   ...
   public class BSScrollingList extends MovieClip
   {
      ...
      public function SetClipByIndex(child:BSScrollingListEntry, index:uint) : Boolean
      {
         var toUpdate:* = this.GetClipByIndex(index);
         if(this.EntryHolder_mc.numChildren <= 0) {
            return false;
         }

         this.EntryHolder_mc.removeChildAt(index);
         this.EntryHolder_mc.addChildAt(child, index);
		 return true;
      }
	  ...
	  // Adjusted as EXTENSION to not accidentally break any weird behaviour
	  public function set selectedEntryExt(updatedEntry:Object)
      {
         this.EntriesA[this.iSelectedIndex] = updatedEntry;
      }
